    ...
    public partial class RagdollProcessor
    {
        ...
        private void PrepareRagdollDummy(Transform objectWithAnimator, Transform rootBone)
        {
            ...
            if (BonesSetupMode == EBonesSetupMode.HumanoidLimbs)
            {
                ...
                if (LeftShoulder) lShould = FTransformMethods.FindChildByNameInDepth(LeftShoulder.name, RagdollDummySkeleton);
                if (RightShoulder) rShould = FTransformMethods.FindChildByNameInDepth(RightShoulder.name, RagdollDummySkeleton);
                if (Chest) chestBone = FTransformMethods.FindChildByNameInDepth(Chest.name, RagdollDummySkeleton);
