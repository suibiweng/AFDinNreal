...
namespace FIMSpace.FProceduralAnimation
{
    ...
    public partial class RagdollProcessor
    {
        ...
        public class BonesChain
        {
            ...
            internal void InitializeForDummy(RagdollProcessor ragdollProcessor, Transform dummyRoot)
            {
                for (int b = 0; b < BoneSetups.Count; b++)
                {
                    ...
                    bone.SetPosingBone(FTransformMethods.FindChildByNameInDepth(bone.t.name, dummyRoot), ragdollProcessor);
        ...
        void Initialization(MonoBehaviour caller, Transform objectWithAnimator, Transform customRagdollAnimator = null, Transform rootBone = null)
        {
            ...
            bool addragd = BlendOnCollision || SendCollisionEventsTo;
            while (c != null)
            {
                if (c.rigidbody != null)
                {
                    ...
                    if (addragd) c.collisions = c.rigidbody.gameObject.AddComponent<RagdollCollisionHelper>().Initialize(this, c);
                }
            ...
            else
            {
                if (BlendOnCollision)
                {
                    ...
                    for (int i = 0; i < CustomLimbsBonesChains.Count; i++)
                    {
                        ...
                        {
                            chain.blendOnCollisionCulldown -= Time.deltaTime;
        ...
        internal void FixedUpdate() // Sync with physics --------------------------------------
        {
            ...
            else if (BonesSetupMode == EBonesSetupMode.CustomLimbs)
            {
                for (int i = 0; i < CustomLimbsBonesChains.Count; i++)
                {
                    ...
                    {
                        var bone = chain.BoneSetups[b];
                        bone.Posing.internalForceMultiplier = chain.MusclesForce;
                    }
                }
            }
        ...
        void UpdateReposing()
        {
            ...
            if (ReposeOnlyOnFullRagdoll) if (RagdolledBlend < 0.99f) return;
